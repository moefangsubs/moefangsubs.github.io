        ...
        for (const key of singleKeys) {
            ...
            function createMemberGroup(title, memberList) {
                ...
                membersArray.forEach(member => {
                    ...
                    if (memberName === "吉田綾乃ｸﾘｽﾃｨｰ") memberName = "吉田綾乃クリスティー";
                    const memberContainer = document.createElement('div'); memberContainer.classList.add('member-container');
                    const memberId = getMemberId(memberName);
                    if(memberId) {
                        const img = document.createElement('img');
                        img.alt = memberName;
                        setMemberImageWithFallback(img, memberId.replace(/-/g, '_'), singleKey);
                        if (title === '選抜復帰') {
