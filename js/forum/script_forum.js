// File: js/script_forum.js (NEW FIX 4)
// Menggunakan Cloud Firestore (db)
// FIX: Menghapus getCountFromServer (fitur v9) dan mengganti logika paginasi

document.addEventListener('DOMContentLoaded', () => {

    // Bug Paste: Izinkan paste di halaman ini
    document.body.classList.add('allow-copy');

    // Cek dependensi Firebase
    ...
    // PERBAIKAN: Hapus getCountFromServer
    const { serverTimestamp, FieldValue } = firebase.firestore;

    // --- 1. Injeksi Kerangka HTML ---
    ...
    // --- 2. Deklarasi Elemen DOM (SETELAH INJEKSI) ---
    ...
    // PERBAIKAN: Hapus totalPages dan totalMessages
    let currentPage = 1;
    let isLastPage = false; // Flag untuk melacak halaman terakhir
    
    let lastVisibleDoc = null; 
    let firstVisibleDoc = null;
    let pageSnapshots = [null]; // Cache untuk [null, firstDocPage1, firstDocPage2, ...]
    ...
    // --- 5. Memuat Pesan (PERBAIKAN: Logika Paginasi Baru) ---
    
    function updatePaginationUI() {
        // PERBAIKAN: Tampilkan "Halaman X"
        const info = `Halaman ${currentPage}`;
        pageInfoTop.textContent = info;
        pageInfoBottom.textContent = info;

        // Tampilkan/sembunyikan berdasarkan halaman
    ...
    async function goToPage(page) {
        ...
        if (page > currentPage && lastVisibleDoc) {
            ...
            // Ini adalah operasi yang mahal, tapi diperlukan
            try {
        ...
        messagesListener = query.onSnapshot(snapshot => {
            ...
            if (snapshot.empty && page === 1) {
                ...
                loadingPlaceholder.style.display = 'block';
                isLastPage = true; // Halaman 1 dan kosong
            } else if (snapshot.empty) {
                // Ini terjadi jika kita klik "Berikutnya" dan tidak ada apa-apa
                isLastPage = true;
                currentPage = page - 1; // Kita sebenarnya di halaman sebelumnya
            } else {
                // PERBAIKAN: Cek apakah ini halaman terakhir
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // PERBAIKAN: Tidak perlu update count, cukup pindah ke hal 1
            if (currentPage !== 1) {
                goToPage(1);
            }
    ...
    // --- 7. Aksi Pesan (Like, Hapus, Pin, Balas) ---
    ...
    confirmBtnPermDeleteAdmin.addEventListener('click', async () => {
        if (messageRefToActOn) {
            try {
                ...
                // Tidak perlu update count, listener akan otomatis refresh
    ...
    // --- 9. Fungsi & Event Listener Toolbar ---
    ...
    // --- 10. Fungsi Transaksi untuk Vote (Anti-Spam) ---
    ...
    async function findAndGoToMessage(messageId) {
        ...
        try {
            ...
            // Kita tidak bisa lagi menggunakan getCountFromServer, jadi kita harus
            // mengambil *semua* dokumen setelahnya untuk menemukan halamannya.
            // Ini bisa lambat jika ada ribuan pesan.
            // Solusi v8 yang lebih baik adalah tanpa "total count".
            
            // PERBAIKAN: Gunakan query `startAt` dan `getCountFromServer` GAGAL.
            // Kita harus memuat semua dokumen (atau setidaknya ID) untuk menghitung.
            // Ini mahal.
            
            // Logika baru yang disederhanakan:
            // Karena kita tidak bisa menghitung dengan mudah, kita coba tebak halamannya.
            // Ini tidak ideal, tapi ini batasan v8 tanpa counter.
            // Untuk sekarang, kita hanya akan muat ulang ke halaman 1 dan berharap 
            // pesan itu ada di sana, atau kita cari manual.
            
            // SOLUSI SEMENTARA: Muat semua ID pesan (mahal!)
            ...
            if (messageIndex === -1) {
                // Mungkin pesan yang di-pin
                const element = document.getElementById(messageId); // Cek lagi
                if (element) {
                    element.scrollIntoView({ behavior: 'smooth', block: 'center' });
                } else {
    ...
    // --- 12. Kontrol Status Login (Auth State) ---
