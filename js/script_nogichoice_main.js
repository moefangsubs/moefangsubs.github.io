document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadUserData() {
        try {
            ...
            if (doc.exists) {
                ...
                if (data.selections) userData.selections = data.selections;  
            } else {
    ...
    function getRomajiName(jpName) {
        ...
        return member ? member.nama_romaji : jpName;  
    }
	...
	async function renderAdminResult() {
        ...
        for (const config of pollSections) {
            ...
            const top3Data = rankedData.slice(0, 3);    
            const midData = rankedData.slice(3, 5);     
            const listData = rankedData.slice(5);       
