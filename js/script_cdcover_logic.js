function getCoverArtUrl(groupKey, singleId, song) {
    ...
    const aobaBaseUrl = 'https://ik.imagekit.io/moearchive/singlealbum/aoba/';
    ...
    switch (groupKey) {
        case 'aobazaka': {
            if (singleId.startsWith('dig')) {
                return `${aobaBaseUrl}a46_cover_audition.jpg`;
            }
            const typeChar = getTypeChar(song.SongTypeAvv);
            return `${aobaBaseUrl}a46_cover_${singleId}${typeChar}.jpg`;
        }

        case 'nogizaka46': {
            if (singleId.startsWith('akb')) {
                const typeChar = getTypeChar(song.songTypeAvv);
